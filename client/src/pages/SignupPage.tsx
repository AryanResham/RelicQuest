...
export default function SignupPage() {
  ...
  const { signUpUser, signInWithOAuth } = useAuthContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setError(typeof result.error === "string" ? result.error : "Sign up failed. Please try again.");
    }
    setLoading(false);
  };
  ...
  const getPasswordStrength = () => {
    if (!password) return { width: "0%", color: "bg-[var(--border)]", text: "" };
    if (password.length < 6) return { width: "33%", color: "bg-[var(--error)]", text: "Weak" };
    if (password.length < 10) return { width: "66%", color: "bg-[var(--warning)]", text: "Medium" };
    return { width: "100%", color: "bg-[var(--success)]", text: "Strong" };
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--background-dark)] px-4">
        <div className="w-full max-w-md">
          <div className="bg-[var(--card-dark)] rounded-2xl p-8 border border-[var(--border)] text-center">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--background-dark)] px-4 py-8">
      <div className="w-full max-w-md">
        {/* Logo */}
        ...
        <div className="bg-[var(--card-dark)] rounded-2xl p-8 border border-[var(--border)]">
          ...
          <p className="text-[var(--text-secondary)] text-center mb-6">
            Join RelicQuest and start bidding
          </p>
              ...
              {password && (
                <div className="mt-2">
                  ...
                  <p className={`text-xs mt-1 ${
                    strength.text === "Weak" ? "text-[var(--error)]" : 
                    strength.text === "Medium" ? "text-[var(--warning)]" : 
                    "text-[var(--success)]"
                  }`}>
                    {strength.text}
                  </p>
